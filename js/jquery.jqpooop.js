	...
	'load':function(urlajax){
		...
		$.ajax({
			...
			success: function(msg)
			{
				...
				$('<div id="'+ opc.Id +'" class="popup"><h6>'+opc.Headmsg+'</h6><button id="botonpop'+ opc.Id +'" onclick="$(this).parent().remove()"></button><p>'+ msg +'</p></div>').appendTo(document.body);
...
'msg':function(msg){
			...
			$('<div id="'+ opc.Id +'" class="popup"><h6>'+opc.Headmsg+'</h6><button id="botonpop'+ opc.Id +'" onclick="$(this).parent().remove()"></button><p>'+ msg +'</p><a href="" class="accept"><span>Aceptar</span></a></div>').appendTo(document.body);
			var w = window.innerWidth ||document.documentElement.clientWidth || document.body.clientWidth;
			var h = window.innerHeight ||document.documentElement.clientHeight || document.body.clientHeight;
			w1 =(w/2) - (opc.Width/2);
			h1 =(h/2) - (opc.Height/2);
