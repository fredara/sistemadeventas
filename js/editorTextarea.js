...
function makeWysiwyg (element) {
  ...
  if (isIE) {
    ...
    doc.write('<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">'
             ...
             +'</head>'
             +'<body style="padding: 5px; margin: 0; spacing: 0; word-wrap: break-word; font-family: Verdana, Arial, Helvetica, sans-serif;	font-size: 12px; color: #333333;">'
             +element.value
             +'&nbsp;</body>'
             +'</html>');
    doc.close();
...
function addImage(src, editor, command, coment) {
  return '<img src="images/'+src+'" onmouseover="seleccionar(this, \'#B6BDD2\');" onmouseout="desseleccionar(this);" onmousedown="seleccionar(this, \'#8592B5\');" onclick="executeCommand(\''+editor+'\', \''+command+'\', event); desseleccionar(this);" style="border: 1px solid #F0F0EE; width; 2px; height: 20px; cursor: pointer;" alt="'+coment+'" title="'+coment+'">';
}

function addEmoticon (src, editor, coment) {
  return '<img src="images/'+src+'" onmouseover="this.style.backgroundColor = \'#B6BDD2\';" onmouseout="this.style.backgroundColor = \'#F0F0EE\';" onclick="executeCommand(\''+editor+'\', \'insertimage\', event, this.src);" style="width; 18px; height: 18px; cursor: pointer;" alt="'+coment+'" title="'+coment+'">';
}
