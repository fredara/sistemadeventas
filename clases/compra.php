<?php
	class Compra{
		...
		var $cod_compra, $nombre_producto, $descripcion, $existencia, $codigo_producto, $cod_foto, $marca;
		var $primero,$ultimo,$total,$proximo,$anterior;

		//constructor de la clase
		function Compra(){
			$this->cod_compra=$this->nombre_producto=$this->descripcion=$this->cantidad="";
		...
		function listaCompras($cod_compra, $pag, $regxpag){
            ...
            mysqli_set_charset($con, "utf8");
            if(!empty($cod_compra)){
                $query="SELECT SQL_CALC_FOUND_ROWS t1.*, date_format(t1.fecha_registro, '%d/%m/%Y') as fecha_1, date_format(t1.fecha_compra, '%d/%m/%Y') as fecha_comp, t2.nombre_usuario, t2.apellido_usuario, t3.nombre_proveedor FROM tbl_compra t1 left join tbl_usuario t2 on (t1.cod_usuario_creo=t2.cod_usuario) left join tbl_proveedor t3 on (t1.cod_proveedor=t3.cod_proveedor) WHERE t1.cod_c$cod_compra = '$cod_compra' ORDER BY t1.cod_c$cod_compra desc LIMIT $inic, $regxpag";
            }else{
                $query="SELECT SQL_CALC_FOUND_ROWS t1.*, date_format(t1.fecha_registro, '%d/%m/%Y') as fecha_1, date_format(t1.fecha_compra, '%d/%m/%Y') as fecha_comp, t2.nombre_usuario, t2.apellido_usuario, t3.nombre_proveedor FROM tbl_compra t1 left join tbl_usuario t2 on (t1.cod_usuario_creo=t2.cod_usuario) left join tbl_proveedor t3 on (t1.cod_proveedor=t3.cod_proveedor)  ORDER BY t1.cod_c$cod_compra desc LIMIT $inic, $regxpag";
            }
